...
$('content').addEventListener('click', async (e) => {
  ...
  if (open) { chrome.tabs.create({ url: open.dataset.open }); return; }
...
chrome.tabs.onActivated.addListener(refreshOrigin);
chrome.tabs.onUpdated.addListener((id, info) => { if (info.status === 'complete') refreshOrigin(); });
refreshOrigin();
